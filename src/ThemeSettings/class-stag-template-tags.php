...
if ( ! class_exists( 'STAG_Template_Tags' ) ) {
	...
	class STAG_Template_Tags {
		...
		/**
		 * Displays the entry tags for the current post.
		 *
		 * @since 1.0.0
		 */
		public static function stag_entry_tags() {
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list();
			if ( $tags_list ) {
				/* translators: 1: list of tags. */

				$tag_list = str_replace( '<a', '<a class="btn--tag"', $tags_list );
				echo '<div class="entry-tags">' . $tag_list . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
